...
SWIGINTERN PyObject *_wrap_GetMinLogLevel(PyObject *SWIGUNUSEDPARM(self), PyObject *args) {
  PyObject *resultobj = 0;
  int result;
  
  if (!SWIG_Python_UnpackTuple(args, "GetMinLogLevel", 0, 0, 0)) SWIG_fail;
  {
    try {
      result = (int)sentencepiece::logging::GetMinLogLevel();
      ReleaseResultObject(resultobj);
    }
    catch (const sentencepiece::util::Status &status) {
      SWIG_exception(ToSwigError(status.code()), status.ToString().c_str());
...
SWIGINTERN PyObject *_wrap_SetMinLogLevel(PyObject *SWIGUNUSEDPARM(self), PyObject *args) {
  PyObject *resultobj = 0;
  int arg1 ;
  int val1 ;
  int ecode1 = 0 ;
  ...
  {
    try {
      sentencepiece::logging::SetMinLogLevel(arg1);
      ReleaseResultObject(resultobj);
    }
    catch (const sentencepiece::util::Status &status) {
      SWIG_exception(ToSwigError(status.code()), status.ToString().c_str());
...
static PyMethodDef SwigMethods[] = {
	 ...
	 { "GetMinLogLevel", _wrap_GetMinLogLevel, METH_NOARGS, NULL},
